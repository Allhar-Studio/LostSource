...
func set_gravity(gravity_value: float):
...
func _keep_decrease_energy(_delta: float):
	if not dead and not is_over_checkpoint:
		energy -= energy_decrease_multiplier * _delta
		energy = clamp(energy, 0, max_energy)
	
func decrease_energy(_amount: float):
	if can_move:
		energy -= _amount
...
func increase_energy(amount: float):
	energy += amount

func recover_energy():
	energy = max_energy

func over_checkpoint():
...
func can_dash() -> bool:
	return energy >= dash_energy_cost and not dash_cooldown
...
func _handle_secondary_texture():
...
func handle_switch(using_energy: bool):
	if using_energy:
		decrease_energy(switch_energy_cost)
	else:
		increase_energy(switch_energy_cost)

func _handle_start_pos():
	var start_pos = Vector2()
	if GameManager.checkpoint["is_active"] == false:
		start_pos = get_parent().get_node("StartPos").position
	else:
		start_pos = GameManager.checkpoint["position"]
...
func _on_DeadlyGroundTimer_timeout():
	decrease_energy(deadly_ground_energy_cost)

func _on_UseEnergyTimer_timeout():
