...
func _on_JumpTutoArea_body_entered(body):
	if body.name == "Player":
		ui.get_node("JumpTutorial").visible = true

func _on_JumpTutoArea_body_exited(body):
	if body.name == "Player":
...
func _on_DoubleJumpArea_body_exited(body):
	if body.name == "Player":
...
func _on_DashArea_body_exited(body):
	if body.name == "Player":
		ui.get_node("Dash").visible = false

func _on_HoldJumpArea_body_entered(body):
	if body.name == "Player":
		ui.get_node("HoldJump").visible = true

func _on_HoldJumpArea_body_exited(body):
	if body.name == "Player":
...
func _on_SwitchArea_body_exited(body):
	if body.name == "Player":
